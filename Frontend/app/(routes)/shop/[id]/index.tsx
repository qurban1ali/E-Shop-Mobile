...
export default function ShopDetailsScreen() {
  ...
  const { data: shop, isLoading: shopLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // normalize image URLs (backend sometimes returns relative paths like "uploads/..." )
      ...
      if (shop) {
        // avatar may be object or string
        if (typeof shop.avatar === "string") {
          shop.avatar = { url: toAbsoluteUrl(shop.avatar) };
        } else if (shop.avatar?.url) {
          shop.avatar.url = toAbsoluteUrl(shop.avatar.url);
        }

        // coverBanner similar
        if (typeof shop.coverBanner === "string") {
          shop.coverBanner = { url: toAbsoluteUrl(shop.coverBanner) };
        } else if (shop.coverBanner?.url) {
          shop.coverBanner.url = toAbsoluteUrl(shop.coverBanner.url);
        }

        // socialLinks urls
        if (Array.isArray(shop.socialLinks)) {
          shop.socialLinks = shop.socialLinks.map((s: any) => ({
            ...s,
            url: s.url ? toAbsoluteUrl(s.url) : s.url,
          }));
        }
      }
  ...
  const { data: products, isLoading: productsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // normalize product images so frontend can use item.images[0].url reliably
      const base = (
        axiosInstance.defaults.baseURL ||
        process.env.EXPO_PUBLIC_SERVER_URI ||
        ""
      ...
      const fixed = products.map((p: any) => {
        if (Array.isArray(p.images)) {
          p.images = p.images.map((img: any) => {
            if (!img) return img;
            if (typeof img === "string") return { url: toAbsoluteUrl(img) };
            if (img.url) return { ...img, url: toAbsoluteUrl(img.url) };
            return img;
          });
        }
        return p;
  ...
  const isFollowing = useMemo(() => {
    ...
    return shop.followers.some(
      (f: any) => String(f?.userId) === String(currentUserId)
    );
  }, [user, shop?.followers]);
  ...
  const handleFollowToggle = async () => {
    ...
    try {
      if (isFollowing) {
        await axiosInstance.post(`/api/v2/shop/unfollow-shop`, {
          shopId: shop?._id || shop?.id,
        });
        toast.success("Unfollowed successfully");
      } else {
        await axiosInstance.post(`/api/v2/shop/follow-shop`, {
          shopId: shop?._id || shop?.id,
        });
  ...
  const handleChat = async () => {
    ...
    try {
      const payload = {
        ...
        groupTilte: `${(user as any)?._id || (user as any)?.id}-${shop?._id || shop?.id
          }`,
      };

      // backend endpoint is /conversation/create-new-conversation
        ...
        <View className="bg-white rounded-3xl p-6 shadow-[0_0_3px_rgba(0,0,0,0.1)] border border-gray-100">
            ...
            <Image
              source={{
                uri:
                  shop?.avatar?.url ||
                  shop?.avatar ||
                  "https://images.unsplash.com/photo-1560472354-b3ff0c44a",
              }}
          ...
          <View className="flex-row items-start">
            ...
            <View className="flex-1 ml-4">
              ...
              <Text className="text-gray-600 text-sm mt-1 font-medium">
                {shop?.description || shop?.bio || "You will get anything related to programing."}
              </Text>
              ...
              <View className="flex-row items-center mt-2">
                ...
                <Text className="text-gray-500 ml-1 text-sm font-medium">
                  {shop?.openning_hours || "Mon - sat: 9 AM- 6 PM"}
                </Text>
              </View>
          ...
          <View className="mt-6 flex-row gap-4">
            <TouchableOpacity
              ...
              disabled={isLoading}
              className={`flex-1 py-4 rounded-2xl ${isFollowing
            ...
            >
              <Text
                className={`text-center font-bold text-base ${isLoading
                  ? "text-gray-400"
                  : isFollowing
                    ? "text-gray-600"
                    : "text-white"
            ...
            >
              <View className="flex-row items-center justify-center">
                ...
                <Text className="text-center font-bold text-base ml-2">
                  {isChatLoading ? "..." : "Chat"}
                </Text>
              </View>
            </TouchableOpacity>
  ...
  const renderProductItem = ({ item }: { item: any }) => {
    const title =
      item?.title || item?.name || item?.productTitle || "Product title";
    const salePrice =
      item?.sale_price ??
      item?.discountPrice ??
      item?.price ??
    ...
    const regularPrice =
      ...
      item?.listPrice;
    const rating = item?.ratings ?? item?.rating ?? "N/A";
    const sold = item?.totalSales ?? item?.sold_out ?? item?.sold ?? 0;
    const imageUri =
      item?.images?.[0]?.url ||
      "https://images.unsplash.com/photo-1505740420928-5e560c0d30e";
      ...
      <TouchableOpacity
        ...
        onPress={() =>
          router.push({
            ...
            params: { id: item._id || item.id || item.slug },
          })
        }
      >
        <View className="relative">
          ...
          <View className="flex-row items-center justify-between mb-2">
            <View className="flex-row items-center">
              ...
              {regularPrice !== undefined && regularPrice !== null && (
                <Text className="text-sm text-gray-400 line-through ml-2 font-medium">
                  ${regularPrice}
                </Text>
              )}
            </View>
  ...
  return (
    <SafeAreaView edges={["bottom"]} className="flex-1 bg-gray-50 ">
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        {/* Header is rendered here; main scrollable lists (FlatList) handle vertical scrolling to avoid nested VirtualizedList inside ScrollView */}
        {renderHeader()}
          ...
          <View className="flex-row bg-gray-100 rounded-2xl p-1">
            {["Products", "Offers", "Reviews"].map((tab) => (
              <TouchableOpacity
                ...
                className={`flex-1 py-3 rounded-xl ${activeTab === tab ? "bg-white" : ""
                  }`}
              >
                <Text
                  className={`text-center font-semibold ${activeTab === tab ? "text-blue-600" : "text-gray-600"
                    }`}
                >
                  {tab}
                </Text>
